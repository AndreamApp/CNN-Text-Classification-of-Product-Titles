...
class RNNConfig(object):
    ...
    embedding_dim = 64      # 词向量维度，'CHAR'模式适用，
                            # 'WORD-NON-STATIC'模式默认为preprocess.py中定义的vec_dim

    layer_num = 1   # rnn层数
    unit_num = 128    # rnn神经元数目
...
class TextRNN(object):
    ...
    def setRNN(self):
        ...
        def basic_rnn_cell():
            rcell = tf.nn.rnn_cell.BasicRNNCell(self.unit_num)
            return tf.nn.rnn_cell.DropoutWrapper(rcell, output_keep_prob=self.dropout_keep_prob)

        def basic_lstm_cell():
            bcell = tf.nn.rnn_cell.BasicRNNCell(self.unit_num)
